...
public class CommMiddleware implements NodeConnectionListener {
    ...
    private static String gatewayIP = "192.168.43.3";

    private static int gatewayPort = 5500;

    private MrUdpNodeConnection connection;

    private UUID myUUID = UUID.fromString("788b2b22-baa6-4c61-b1bb-01cff1f5f879");
    ...
    public void newMessageReceived(NodeConnection remoteCon, Message message) {
        if (message.getContentObject() instanceof String) {
            final char[] msg = message.getContentObject().toString().toCharArray();
            String preamble = String.valueOf(msg, 0, 4);
            if (preamble.equals(COMMUNICATOR_PREAMBLE)) {
                String firstParam = EMPTY_VALUE;
                for (int cont = 6; cont < (this.hex2int(char2int(msg[5]), char2int(msg[4])) + 6); cont++) {
                    firstParam += msg[cont];
                }
    ...
    /**
     * Trata as mensagens em texto de transferência de agentes. As mensagens estão no padrão [fffe][ss][protocol][ss][value1][ss][value2]...
     *
    ...
    private void treatAgentTransferenceMsg(char[] message) {
        ...
        int currentValueIndex = currentMsgIndex + 2; // Índice do início do primeiro value [fffe][ss][protocolo][ss][value].
        int currentValueSize = this.hex2int(char2int(message[currentMsgIndex + 1]), char2int(message[currentMsgIndex])); // Tamanho do value.
        int finalPosition = 0;
